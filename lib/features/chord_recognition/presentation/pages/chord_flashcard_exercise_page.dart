...
import 'package:key_starter/features/chord_recognition/presentation/widgets/chord_flashcard_staff_card.dart';
...
class ChordFlashcardExercisePage extends ConsumerStatefulWidget {
  final NoteExerciseSettings settings;

  const ChordFlashcardExercisePage({super.key, required this.settings});

  @override
  ConsumerState<ChordFlashcardExercisePage> createState() =>
      _ChordFlashcardExercisePageState();
}

class _ChordFlashcardExercisePageState
    extends ConsumerState<ChordFlashcardExercisePage> {
  ...
  Widget build(BuildContext context) {
    ...
    final exerciseState = ref.watch(
      chordFlashcardExerciseProvider(widget.settings),
    );

    ref.listen(chordFlashcardExerciseProvider(widget.settings), (_, next) {
      if (next is ChordFlashcardExerciseCompleted) {
        if (!mounted) return;
