...
import 'package:key_starter/features/note_recognition/presentation/widgets/defilement_staff_card.dart';
...
class DefilementExercisePage extends ConsumerStatefulWidget {
  final NoteExerciseSettings settings;

  const DefilementExercisePage({super.key, required this.settings});

  @override
  ConsumerState<DefilementExercisePage> createState() =>
      _DefilementExercisePageState();
}

class _DefilementExercisePageState
    extends ConsumerState<DefilementExercisePage> {
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen(defilementExerciseProvider(widget.settings), (_, next) {
      if (next is DefilementExerciseCompleted) {
        if (!mounted) return;
        ...
        navigator.pushReplacement(
          MaterialPageRoute(
            builder: (_) => RecapPage(
              exerciseLabel: 'Lecture · défilement',
    ...
    final notifier = ref.read(
      defilementExerciseProvider(widget.settings).notifier,
    );
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              DefilementStaffCard(settings: widget.settings),
              const Spacer(),
              Row(
                children: [
                  Expanded(
