...
final _createSessionUseCaseProvider = Provider<CreateSessionUseCase>(
  (_) => sl<CreateSessionUseCase>(),
);

final _getLastSessionParamsUseCaseProvider =
    Provider<GetLastSessionParamsUseCase>(
      (_) => sl<GetLastSessionParamsUseCase>(),
    );
...
class SessionSetupNotifier extends Notifier<SessionSetupState> {
  ...
  Note _stepToNote(int step) {
    ...
    return Note(
      midiNumber: (octave + 1) * 12 + diatonicSemitones[noteIndex],
      name: noteNamesEn[noteIndex],
      octave: octave,
    );
  }

  int _noteToStep(Note note) {
    final noteIndex = noteNamesEn.indexOf(note.name);
